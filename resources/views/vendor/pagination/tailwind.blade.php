@if ($paginator->hasPages())
    <nav class="flex justify-center" role="navigation">
        @if ($paginator->onFirstPage())
            <span class="px-4 py-2 bg-gray-300 text-gary-500 rounded-l-lg">Previous</span>
        @else
        ...
        @foreach ($elements as $element)
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @else
                        <a href="{{$url}}" class="px-4 py-2 bg-gray-200 hover:bg-blue-600 text-gray-700 hover:text-white">{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
        ...
        @else
            <span class="px-4 py-2 bg-gray-300 text-gary-500 rounded-r-lg">Next</span>
        @endif
    </nav>
@else
